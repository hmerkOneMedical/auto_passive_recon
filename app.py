...
def tests():
    ...
    domainData = [{'url': 'https://www.wikipedia.com', 'ip':'0.0.0.0', 'vulns':'123-Abs'}]
    return render_template('tests.html', data=jobs, domainData=domainData)
...
def recon():
    ...
    jobs = response['jobs']
    founderEmails = response['founderEmails']
    ...
    return render_template('report_details.html', companyName=companyName, details=details, jobs=jobs, employees=employees, domainData=domainData, founderEmails=founderEmails)
